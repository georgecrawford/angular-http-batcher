...
function getAdapterFn() {
    ...
    if (typeof adapter === 'object') {
        if (
            adapter.buildRequest === undefined ||
            adapter.parseResponse === undefined
        ) {
            throw new Error(
                'A custom adapter must contain the methods "buildRequest" and "parseResponse" - please see the docs'
            );
        }
        ...
        if (adapter === undefined) {
            throw new Error(
                'Unknown type of http batch adapter: ' + this.config.adapter
            );
        }
    }
...
function sendFn() {
    ...
    self.sendCallback();
    self.$injector
        .get('$http')(httpBatchConfig)
        .then(
            function (response) {
                var batchResponses = adapter.parseResponse(
                    self.requests,
                    response,
                    self.config
                );
...
function BatchRequestManager(
    ...
    this.currentTimeoutToken = $timeout(
        function () {
        ...
        config.batchRequestCollectionDelay,
        false
    );
}
...
function HttpBatcherFn(
    ...
    function batchRequestFn(request) {
        var batchConfig = httpBatchConfig.getBatchConfig(request.url),
            batchRequestManager =
                currentBatchedRequests[batchConfig.batchEndpointUrl];
    ...
    function flushInternalFn(batchEndpointUrl) {
        angular.forEach(currentBatchedRequests, function (val, key) {
            var shouldFlush =
                batchEndpointUrl === undefined ||
                (batchEndpointUrl &&
                    key.toLocaleLowerCase() ===
                        batchEndpointUrl.toLocaleLowerCase());
            if (shouldFlush) {
                val.flush();
            }
        });
